                        ...
                        $v.find('td').each(function(){
                            ...
                            this.className.split(" ").forEach(function(v){
                                if(v.indexOf("column-")===0){
                                    if(thead.find("th."+v).hasClass("hidden")){td.addClass("hidden");}
                                }
                            });
                        });
                    });
...
// end of extract from .../wp-includes/js/media-views.js
    ...
    jQuery("form#adv-settings").prepend(
        '<fieldset class="tml_options"><legend>TML Specific Options</legend>'+
        '<label for="nggtags-for-ml-use-checkboxes-overlay"><input type="checkbox" id="nggtags-for-ml-use-checkboxes-overlay">Use Checkbox Overlay for Filters</label>'+
        '<label for="nggtags-for-ml-use-alt-media-list-pane"><input type="checkbox" id="nggtags-for-ml-use-alt-media-list-pane">Use Alternate High Density Media List View</label>'+
        '<label for="nggtags-sort-by-priority"><input type="radio" name="nggml-sort-by" id="nggtags-sort-by-priority" value="priority">Sort by Priority</label>'+
        '<label for="nggtags-sort-by-date"><input type="radio" name="nggml-sort-by" id="nggtags-sort-by-date" value="date">Sort by Date</label>'+
        '<label for="nggtags-sort-by-title"><input type="radio" name="nggml-sort-by" id="nggtags-sort-by-title" value="title">Sort by Title</label>'+
        '</fieldset>'
    );
    ...
    // normalize the filter selection
    jQuery("select.nggml-filter").change(function(e){
        var select=jQuery(this);
        var selected=select.find("option").filter(":selected");
        var optionAll=select.find("option[value='0-nggml-all']");
        if(selected.length){
            if(selected.not("[value='0-nggml-all']").length){
                optionAll.prop("selected",false);
            }
        }else{
            optionAll.prop("selected",true);
        }
        if(!optionAll.prop("selected")){
    ...
    jQuery("input#nggtags-sort-by-priority").prop("checked",nggmlScreenOptions.sort_by==='priority');
    jQuery("input#nggtags-sort-by-date").prop("checked",nggmlScreenOptions.sort_by==='date');
    jQuery("input#nggtags-sort-by-title").prop("checked",nggmlScreenOptions.sort_by==='title');
    jQuery("input[name='nggml-sort-by']").change(function(){
        if(this.checked){
            nggmlScreenOptions.sort_by=this.value;
            jQuery.post(ajaxurl,{action:'nggml_update_screen_options',nggml_screen_options:nggmlScreenOptions},function(data){
                window.location.assign(window.location.origin+window.location.pathname+"?page=tml_upload&sort="+nggmlScreenOptions.sort_by);
            });
        }
    }); 
    ...
    function showAltMediaListPane(){
        ...
        var imagesWidth=0.79*jQuery("table.widefat tbody").width()-20;
        var width=Math.floor(imagesWidth/Math.floor(imagesWidth/(80+22)))-22;
        var posts=new Array();
        jQuery("table.wp-list-table tbody#the-list tr[id|='post']").each(function(){
            ...
            var title=jQuery(this).find("td.title strong a").text();
            var image=jQuery("td.title a span.media-icon img",this).get(0);
            ...
            var liLock=document.createElement("div");
            liLock.className="nggml-li-lock";
            liLock.style.display="inline-block";
            li.appendChild(liLock);
            ...
            imgCheckbox.style.left="0";
            imgCheckbox.style.top="0";
            imgCheckbox.style.zIndex=100;
            imgCheckbox.style.margin="0";
        ...
        var rowHeight=null;
        jQuery("fieldset#nggml-alt-media-list-pane div#nggml-alt-media-list-images div.nggml-image-icon-box div.nggml-img-lock").click(function(){
            if(!media.nggml.nggmlImageIconLocked){
                this.style.color="red";
                media.nggml.nggmlImageIconLocked=true;
                jQuery("li.nggml-image-title-box.nggml-focused").find("div.nggml-li-lock").css("color","red");
                ...
                tr.css("height","auto");
                tr.css("background-color","#ffd0ff");
            }else if(this.style.color==="red"){
                this.style.color="black";
                media.nggml.nggmlImageIconLocked=false;
                jQuery("li.nggml-image-title-box.nggml-focused").find("div.nggml-li-lock").css("color","black");
                var id=jQuery("div.nggml-image-icon-box.nggml-focused")[0].id.substr(10);
                var tr=jQuery("table.wp-list-table tbody#the-list tr[id='post-"+id+"']");
        ...
        jQuery("fieldset#nggml-alt-media-list-pane div#nggml-alt-media-list-titles li.nggml-image-title-box div.nggml-li-lock").click(function(){
            if(!media.nggml.nggmlImageIconLocked){
                this.style.color="red";
                media.nggml.nggmlImageIconLocked=true;
                jQuery("div.nggml-image-icon-box.nggml-focused").find("div.nggml-img-lock").css("color","red");
                var id=jQuery("li.nggml-image-title-box.nggml-focused")[0].id.substr(9);
                var tr=jQuery("table.wp-list-table tbody#the-list tr[id='post-"+id+"']");
                rowHeight=tr.height();
                tr.find("td.title div.row-actions").css("visibility","visible").show();
                tr.css("height","auto");
                ...
                this.style.color="black";
                media.nggml.nggmlImageIconLocked=false;
                jQuery("div.nggml-image-icon-box.nggml-focused").find("div.nggml-img-lock").css("color","black");
                var id=jQuery("li.nggml-image-title-box.nggml-focused")[0].id.substr(9);
                var tr=jQuery("table.wp-list-table tbody#the-list tr[id='post-"+id+"']");
                tr.find("td.title div.row-actions").css("visibility","hidden").hide();
                tr.height(rowHeight);
        ...
        jQuery("fieldset#nggml-alt-media-list-pane div#nggml-alt-media-list-images div.nggml-image-icon-box \
            ...
            jQuery("table.wp-list-table tbody#the-list tr#post-"+id+" input[type='checkbox']#cb-select-"+id)
                .prop("checked",this.checked);
            if(!this.checked){
                var selectAll=jQuery("input[type='checkbox'][id|='cb-select-all']");
...
*/
  ...
  cleanTagsInitial:function(){
    ...
    cleanImages:function(){
        ...
        var height=0;
        jQuery('table.wp-list-table tbody#the-list td.column-primary strong.has-media-icon img').each(function(){
