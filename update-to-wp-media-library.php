...
and the NextGEN Gallery shortcodes will now work with the Media Library images.
Except for 'gallery' and 'album' the parameters to the NextGEN Gallery shortcodes are parameters to
WordPress's 'gallery' shortcode.
                ...
                foreach ( $results as $result ) {
                    ...
                    if ( !in_array( $result->gid, $done_gids ) ) {
                        ...
                        if ( $id = wp_insert_post( $post ) ) {
                            // The post id is now known so we can refer to it in the post content
...
<!--
    ...
    This gallery is intended to be displayed using NextGen Gallery's nggallery or slideshow shortcodes.
         [nggallery id="{$id}"] or [slideshow id="{$id}"]
    It can also be displayed using the WordPress gallery shortcode.
         [gallery id="{$id}"]
...
SELECT ID, post_content FROM $wpdb->posts
    WHERE ( post_content LIKE '%[nggallery %' OR post_content LIKE '%[singlepic %' OR post_content LIKE '%[singlepic=%'
        ...
        AND post_type != 'tml_galleries_albums'
